...
| `MANGROVE_CUSTOM_DOMAIN_MODE` | `auto_tls` | `auto_tls` (default) verifies a new custom domain via DNS TXT record and serves it off Caddy's own automatic-HTTPS `:80`/`:443` block. Set to `port` on a box that isn't reachable on `:80`/`:443` directly (e.g. Hack Club Nest) -- a new domain then skips verification and gets a dedicated port instead, routed like a deployment's own base port; register `domain -> port` in your host's own dashboard. See [docs/deployment.md](docs/deployment.md#custom-domains-on-nest-or-anywhere-else-80443-isnt-reachable) |
| `MANGROVE_PREVIEW_MAX_AGE_HOURS` | `168` (a week) | Tears down a PR-preview deployment that's had no new push in this many hours -- a backstop for a missed/never-delivered "PR closed" webhook, or a PR just left open, either of which would otherwise leave the preview eating the deployment-memory admission budget indefinitely. `0` disables the sweep |
