...
func TestCreateAndValidateSession(t *testing.T) {
	...
	userID, err := st.CreateUser(ctx, "evan@example.com", hash)
	if err != nil {
	...
	gotUserID, err := ValidateSession(ctx, st, token)
	if err != nil {
...
func TestValidateSessionRejectsUnknownToken(t *testing.T) {
	st := testStore(t)
	if _, err := ValidateSession(context.Background(), st, "not-a-real-token"); err == nil {
...
func TestDeleteSessionInvalidatesToken(t *testing.T) {
	...
	hash, _ := HashPassword("password123")
	userID, _ := st.CreateUser(ctx, "evan@example.com", hash)
	token, err := CreateSession(ctx, st, userID, "127.0.0.1", "test-agent")
	if err != nil {
		t.Fatalf("CreateSession: %v", err)
	}
	...
	if _, err := ValidateSession(ctx, st, token); err == nil {
