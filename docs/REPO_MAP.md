...
| `internal/db/` | `db.go` (connection, WAL, migration runner) + `migrations/*.sql` (numbered, forward-only). The runner's post-migration `foreign_key_check` only fails on violations the migration *introduces*; ones already in the DB are logged and left alone (a prod orphan once crash-looped a deploy — see `TestMigrationToleratesPreExistingFKViolation`). | — |
